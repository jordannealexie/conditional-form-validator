...
class CasbinEnforcer:
    ...
    def _initialize_sync(self):
        ...
        try:
            ...
            print("✅ RBAC/ABAC enforcers initialized successfully")
    ...
    def _initialize_fallback_sync(self):
        ...
        try:
            ...
            # Initialize enforcers with same adapter
    ...
    def enforce_unified(self, user: any, resource: str, action: str, resource_obj: any = None) -> bool:
        """
        Unified enforcement across RBAC and ABAC.
        
        Logic (STRICT):
        1. Check Superuser (bypass)
        2. Check RBAC (Roles) -> if denied, stop
        3. Check ABAC (Attributes) -> must pass
        
        Returns True only if RBAC passes AND ABAC passes.
        """
        # 0. Superuser check
        ...
        if not rbac_result:
            print(f"✗ Access denied for {username}: RBAC denied")
            return False
        ...
        if abac_result:
            print(f"✓ Access granted for {username}: RBAC+ABAC")
            return True

        print(f"✗ Access denied for {username}: ABAC denied")
        return False
